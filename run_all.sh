...
# Loads all 4 datasets to their targets and verifies ingestion end-to-end.
#
# Usage:
#   ./run_all.sh                  # Full run: generate + load + verify
#   ./run_all.sh --generate       # Generate datasets only
#   ./run_all.sh --load-pg        # Load PostgreSQL only (members, accounts, loans, txns, regulatory)
...
for arg in "$@"; do
    ...
    case $arg in
        ...
        --test)       TEST_ONLY=true; DO_PG=true; DO_S3=true; DO_BQ=true; DO_KAFKA=true ;;
        --dry-run)    DRY_RUN=true ;;
        --help|-h)
            echo "Usage: $0 [--generate] [--load-pg] [--load-s3] [--load-bq] [--load-kafka] [--verify] [--test] [--dry-run]"
            exit 0 ;;
...
if $DO_ALL; then
    DO_GENERATE=true; DO_PG=true; DO_S3=true; DO_BQ=true; DO_KAFKA=true; DO_VERIFY=true
fi
...
DEPS_OK=true
check_pkg psycopg2 psycopg2-binary   || DEPS_OK=false
...
if ! $DEPS_OK; then
    ...
    echo "    pip install psycopg2-binary boto3 google-cloud-bigquery kafka-python requests"
    echo ""
fi
...
if $DO_GENERATE; then
    ...
    ok "Datasets generated (members, accounts, loans, transactions)"
    ((PASSED++))
fi

# ═══════════════════════════════════════════════════════════════
# STEP 3: Load to PostgreSQL (primary data + regulatory seeding)
# ═══════════════════════════════════════════════════════════════
if $DO_PG; then
    echo ""
    log "STEP 2: Loading all data → PostgreSQL..."
    echo "─────────────────────────────────────────────"
    PG_ARGS=""
    if $TEST_ONLY; then PG_ARGS="--test"; fi

    if python3 "$SCRIPT_DIR/scripts/load_postgres.py" $PG_ARGS; then
        ok "PostgreSQL load complete (members, accounts, loans, txns, snapshots, alerts, regulatory)"
        ((PASSED++))
    else
        fail "PostgreSQL load failed"
        ((FAILED++))
    fi
fi

# ═══════════════════════════════════════════════════════════════
# STEP 4: Load to AWS S3
# ═══════════════════════════════════════════════════════════════
if $DO_S3; then
    echo ""
    log "STEP 3: Loading members + loans → AWS S3..."
    echo "─────────────────────────────────────────────"
    S3_ARGS=""
...
if $DO_BQ; then
    echo ""
    log "STEP 4: Loading accounts → Google BigQuery..."
    echo "─────────────────────────────────────────────"
...
# ═══════════════════════════════════════════════════════════════
# STEP 6: Stream to Kafka
# ═══════════════════════════════════════════════════════════════
if $DO_KAFKA; then
    echo ""
    log "STEP 5: Streaming transactions → Apache Kafka..."
    echo "─────────────────────────────────────────────"
...
if $DO_VERIFY && ! $TEST_ONLY; then
    echo ""
    log "STEP 6: Verifying ingestion..."
    echo "─────────────────────────────────────────────"
...
echo "  └──────────────┘    └───────────────┘    │                          │"
echo "  ┌──────────────┐    ┌───────────────┐    │    Pinot Pulse           │"
echo "  │accounts.ndjsn│───>│  BigQuery     │───>│    Enterprise            │"
echo "  │ (800 rows)   │    │  Warehouse    │    │                          │"
echo "  └──────────────┘    └───────────────┘    │  ┌─────────┐ ┌────────┐ │"
echo "  ┌──────────────┐    ┌───────────────┐    │  │ Apache  │ │Postgres│ │"
echo "  │ txns.jsonl   │───>│  Kafka Topic  │───>│  │ Pinot   │ │  + 9   │ │"
echo "  │ (5000 rows)  │    │  Real-Time    │    │  │         │ │  reg   │ │"
echo "  └──────────────┘    └───────────────┘    │  └─────────┘ │ tables │ │"
echo "  ┌──────────────┐    ┌───────────────┐    │              └────────┘ │"
echo "  │ loans.json   │───>│  PostgreSQL   │───>│  analytics.loans       │"
echo "  │ (350 rows)   │    │  Direct Load  │    │  + regulatory schema   │"
echo "  └──────────────┘    └───────────────┘    └──────────────────────────┘"
echo ""
