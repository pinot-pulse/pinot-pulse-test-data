...
Checks that data was successfully loaded to S3, BigQuery, Kafka, and PostgreSQL,
then verifies Pinot Pulse processed it into Apache Pinot + PostgreSQL.
Includes verification for regulatory tables (migration 013).
...
Usage:
  ...
  python3 verify_ingestion.py --postgres         # PostgreSQL regulatory tables
...
EXPECTED = {"members": 500, "accounts": 800, "loans": 350, "transactions": 5000}
...
def check_postgres(args):
    print(f"\n{'='*50}")
    print(f"  PostgreSQL — Analytics & Regulatory Tables")
    print(f"{'='*50}")
    try:
        import psycopg2
        host = os.getenv("POSTGRES_HOST", "localhost")
        port = os.getenv("POSTGRES_PORT", "5433")
        user = os.getenv("POSTGRES_USER", "pinot_pulse")
        password = os.getenv("POSTGRES_PASSWORD", "pulse_secure_2024")
        database = os.getenv("POSTGRES_DB", "pinot_pulse")
        info(f"Host: {host}:{port}, Database: {database}")
        ...
        # Analytics tables
        analytics_tables = {
            "analytics.members": EXPECTED["members"],
            "analytics.accounts": EXPECTED["accounts"],
            "analytics.loans": EXPECTED["loans"],
            "analytics.transactions": EXPECTED["transactions"],
            "analytics.dashboard_snapshots": 12,
            "analytics.fraud_alerts": 25,
            "analytics.report_generations": 7,
        }
        for table, expected in analytics_tables.items():
            try:
                cur.execute(f"SELECT COUNT(*) FROM {table}")
                count = cur.fetchone()[0]
                if count >= expected:
                    ok(f"{table}: {count:,} rows (expected >={expected})")
                elif count > 0:
                    warn(f"{table}: {count:,} rows (expected >={expected})")
                else:
                    fail(f"{table}: 0 rows (expected >={expected})")
            except Exception as e:
                fail(f"{table}: {e}")
                conn.rollback()

        # Regulatory tables (migration 013)
        print(f"\n  Regulatory Schema (migration 013):")
        regulatory_tables = [
            "regulatory.filing_records",
            "regulatory.filing_approvals",
            "regulatory.exam_records",
            "regulatory.exam_findings",
            "regulatory.audit_periods",
            "regulatory.audit_reports",
            "regulatory.evidence_items",
            "regulatory.evidence_packages",
            "regulatory.evidence_package_items",
        ]
        for table in regulatory_tables:
            try:
                cur.execute(f"SELECT COUNT(*) FROM {table}")
                count = cur.fetchone()[0]
                ok(f"{table}: {count:,} rows (table exists)")
            except Exception as e:
                fail(f"{table}: {e}")
                conn.rollback()
        ...
        try:
            ...
            if policies:
                ok(f"Found {len(policies)} RLS policies on regulatory tables")
                tables_with_rls = set(p[0] for p in policies)
                info(f"Tables with RLS: {', '.join(sorted(tables_with_rls))}")
            else:
                warn("No RLS policies found on regulatory tables")
        except Exception as e:
            warn(f"RLS check: {e}")
            conn.rollback()
        ...
        warn("psycopg2 not installed — skipping PostgreSQL check (pip install psycopg2-binary)")
    except Exception as e:
        fail(f"PostgreSQL check failed: {e}")
...
def main():
    ...
    parser.add_argument("--postgres", action="store_true", help="PostgreSQL regulatory tables")
    parser.add_argument("--api", action="store_true")
    args = parser.parse_args()

    # Default to --all if nothing specified
    if not any([args.all, args.s3, args.bigquery, args.kafka, args.pinot, args.postgres, args.api]):
        args.all = True

    print("═══ Pinot Pulse Enterprise — Ingestion Verification ═══")
    print(f"  Expected: {EXPECTED['members']} members, {EXPECTED['accounts']} accounts, "
          f"{EXPECTED['loans']} loans, {EXPECTED['transactions']} transactions")
    ...
    if args.all or args.postgres:  check_postgres(args)
